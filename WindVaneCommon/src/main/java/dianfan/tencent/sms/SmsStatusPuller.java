...
public class SmsStatusPuller {
	...
	public HttpURLConnection constructConnection(int type, int max) throws Exception {
		/*
		 * { "sig": "xxxxxx", // sha256(appkey=$appkey&rand=$rand&time=$time) "type": 0,
		 * // 类型 "max": 10, //最大条数 "time": 1464624000
		 * //unix时间戳，请求发起时间，如果和系统时间相差超过10分钟则会拉取失败 }
		 */
		// 按照协议组织 post 请求包体
		...
		data.put("sig", util.strToHash(String.format("appkey=%s&random=%d&time=%d", appkey, random, curTime)));
